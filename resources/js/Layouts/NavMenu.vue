<template>
	<!-- component -->
<!-- <div class="rounded-r bg-gray-900 xl:hidden flex justify-between w-1/2 p-6 items-center ">
...
<div id="Main" class="xl:rounded-r transform  xl:translate-x-0  ease-in-out transition duration-500 flex justify-start items-start h-full  w-full sm:w-64 bg-gray-900 flex-col">
  ...
  <div class="flex flex-col justify-start items-center   px-6 border-b border-gray-600 w-full  ">
    <button @click="toggleMenu(1)" class="focus:outline-none focus:text-indigo-400 text-left  text-white flex justify-between items-center w-full py-5 space-x-14  ">
...
export default {
  ...
  data(){
    return{
      isNavHidden: true, // State untuk menyembunyikan atau menampilkan nav
      isIcon1Rotated: false,
      isIcon2Rotated: false,
      isIcon3Rotated: false,
    }
  },
  methods: {
    toggleNav(flag) {
    ...
    toggleMenu(menuNumber) {
      if (menuNumber === 1) {
        this.isIcon1Rotated = !this.isIcon1Rotated;
        this.$refs.menu1.classList.toggle("hidden"); // Menyembunyikan/menampilkan menu1
      } else if (menuNumber === 2) {
        this.isIcon2Rotated = !this.isIcon2Rotated;
      } else if (menuNumber === 3) {
        this.isIcon3Rotated = !this.isIcon3Rotated;
      }
    },
  },
};
